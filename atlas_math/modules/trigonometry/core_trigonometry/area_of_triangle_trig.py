...
MODULE_INFO = {
    "module_id": "trigonometry.area_of_triangle_trig",
    "name": "Area of Triangle with Trigonometry",
    "topic": "trigonometry",
...
INSTRUCTIONS = [
    "Find the area of the triangle: {problem}",
    "Use the trig area formula to solve: {problem}",
    "Compute the triangle's area: {problem}",
]

def _fmt(v: float) -> str:
    return str(int(v)) if abs(v-round(v)) < 1e-9 else f"{v:.2f}".rstrip('0').rstrip('.')

def _emit(level, sides, angles):
    count = 0
    cap = LEVEL_SPEC_CAPS[level]
    for a in sides:
        for b in sides:
            for deg, sval in angles:
                area = 0.5 * a * b * sval
                prompt = f"A triangle has sides {a} and {b} with included angle {deg}°. Find its area."
                yield _spec("sas_area", (a,b,deg), prompt, _fmt(area), {"unit": "square units"})
                count += 1
                if count >= cap:
                    return

def iter_level1_specs(): return _emit("level_1", range(2,11), [(30,0.5)])
def iter_level2_specs(): return _emit("level_2", range(3,13), [(45,2**0.5/2)])
def iter_level3_specs(): return _emit("level_3", range(4,15), [(60,3**0.5/2)])
def iter_level4_specs(): return _emit("level_4", range(5,17), [(30,0.5),(45,2**0.5/2),(60,3**0.5/2)])
def iter_level5_specs(): return _emit("level_5", range(6,19), [(120,3**0.5/2),(135,2**0.5/2),(150,0.5)])

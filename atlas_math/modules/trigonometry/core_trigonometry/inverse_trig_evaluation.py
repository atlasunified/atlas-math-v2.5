...
MODULE_INFO = {
    "module_id": "trigonometry.inverse_trig_evaluation",
    "name": "Inverse Trig Evaluation",
    "topic": "trigonometry",
...
INSTRUCTIONS = [
    "Evaluate the inverse trig expression: {problem}",
    "Find the principal value: {problem}",
    "Compute the exact angle: {problem}",
]

def _emit(level, entries):
    count = 0
    cap = LEVEL_SPEC_CAPS[level]
    for fn, inp, out in entries:
        prompt = f"Evaluate {fn}({inp}). Give the principal value in radians."
        yield _spec("principal_value", (fn, inp), prompt, out)
        count += 1
        if count >= cap:
            return

def iter_level1_specs():
    return _emit("level_1", [("arcsin","0","0"),("arcsin","1/2","π/6"),("arccos","1","0"),("arctan","0","0")])
def iter_level2_specs():
    return _emit("level_2", [("arcsin","√2/2","π/4"),("arccos","1/2","π/3"),("arctan","1","π/4")])
def iter_level3_specs():
    return _emit("level_3", [("arcsin","√3/2","π/3"),("arccos","√2/2","π/4"),("arctan","√3","π/3")])
def iter_level4_specs():
    return _emit("level_4", [("arcsin","-1/2","-π/6"),("arccos","-1/2","2π/3"),("arctan","-1","-π/4")])
def iter_level5_specs():
    return _emit("level_5", [("arcsin","-√2/2","-π/4"),("arccos","-√3/2","5π/6"),("arctan","-√3","-π/3")])

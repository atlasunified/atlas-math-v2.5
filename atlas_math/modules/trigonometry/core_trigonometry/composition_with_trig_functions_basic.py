...
MODULE_INFO = {
    "module_id": "trigonometry.composition_with_trig_functions_basic",
    "name": "Composition with Trig Functions Basic",
    "topic": "trigonometry",
...
INSTRUCTIONS = [
    "Evaluate the composite trig expression: {problem}",
    "Compute the exact value: {problem}",
    "Simplify and evaluate: {problem}",
]

def _emit(level, outer, inner_vals):
    cap = LEVEL_SPEC_CAPS[level]
    count = 0
    exact = {0:("0","1","undefined"), 30:("1/2","√3/2","√3/3"), 45:("√2/2","√2/2","1"), 60:("√3/2","1/2","√3")}
    for op in outer:
        for angle in inner_vals:
            s,c,t = exact[angle]
            val = {"sin":s,"cos":c,"tan":t}[op]
            prompt = f"Evaluate {op}({angle}°)."
            yield _spec("basic_eval", (op, angle), prompt, val)
            count += 1
            if count >= cap:
                return

def iter_level1_specs(): return _emit("level_1", ["sin"], [0,30,45])
def iter_level2_specs(): return _emit("level_2", ["cos"], [0,30,45,60])
def iter_level3_specs(): return _emit("level_3", ["tan"], [30,45,60])
def iter_level4_specs(): return _emit("level_4", ["sin","cos"], [30,45,60])
def iter_level5_specs(): return _emit("level_5", ["sin","cos","tan"], [0,30,45,60])

...
MODULE_INFO = {
    "module_id": "precalculus.function_composition",
...
INSTRUCTIONS = [
    "Find the composition: {problem}",
    "Compute the composed function or value: {problem}",
    "Evaluate the composition: {problem}",
]

def _emit(level, defs, eval_only=False):
    cap = LEVEL_SPEC_CAPS[level]
    for idx, item in enumerate(defs):
        if idx >= cap:
            return
        fexpr, gexpr, xval, ans = item
        if eval_only:
            prompt = f"Let f(x) = {fexpr} and g(x) = {gexpr}. Find (f∘g)({xval})."
            yield _spec("compose_eval", (fexpr,gexpr,xval), prompt, ans)
        else:
            prompt = f"Let f(x) = {fexpr} and g(x) = {gexpr}. Find (f∘g)(x)."
            yield _spec("compose_symbolic", (fexpr,gexpr), prompt, ans)

def iter_level1_specs(): return _emit("level_1", [("x+2", "x-1", None, "x+1"), ("2x", "x+3", None, "2x+6")])
def iter_level2_specs(): return _emit("level_2", [("x^2", "x+1", None, "(x+1)^2"), ("3x-4", "2x", None, "6x-4")])
def iter_level3_specs(): return _emit("level_3", [("x+5", "x^2", 3, "14"), ("2x-1", "x+4", -2, "3")], True)
def iter_level4_specs(): return _emit("level_4", [("x^2+1", "x-3", 5, "5"), ("3x+2", "x^2", -1, "5")], True)
def iter_level5_specs(): return _emit("level_5", [("2x+1", "x^2-4", None, "2x^2-7"), ("x^2-1", "3x", None, "9x^2-1")])

...
MODULE_INFO = {
    "module_id": "precalculus.polynomial_end_behavior",
    "name": "Polynomial End Behavior",
    "topic": "precalculus",
...
INSTRUCTIONS = [
    "State the end behavior: {problem}",
    "Describe what happens as x→∞ and x→-∞: {problem}",
    "Determine the polynomial's end behavior: {problem}",
]

def _emit(level, rows):
    cap = LEVEL_SPEC_CAPS[level]
    for idx, (expr, answer) in enumerate(rows):
        if idx >= cap:
            return
        yield _spec("poly_end_behavior", (expr,), f"Describe the end behavior of f(x) = {expr}.", answer)

def iter_level1_specs():
    return _emit("level_1", [
        ("x^2 + 3x - 1", "as x→-∞, f(x)→∞; as x→∞, f(x)→∞"),
        ("-2x^2 + 5", "as x→-∞, f(x)→-∞; as x→∞, f(x)→-∞"),
    ])

def iter_level2_specs():
    return _emit("level_2", [
        ("x^3 - 4x", "as x→-∞, f(x)→-∞; as x→∞, f(x)→∞"),
        ("-3x^3 + x - 2", "as x→-∞, f(x)→∞; as x→∞, f(x)→-∞"),
    ])

def iter_level3_specs():
    return _emit("level_3", [
        ("5x^4 - x^2 + 7", "as x→-∞, f(x)→∞; as x→∞, f(x)→∞"),
        ("-x^5 + 2x^3 - 9", "as x→-∞, f(x)→∞; as x→∞, f(x)→-∞"),
        ("7x^6 - 2x + 1", "as x→-∞, f(x)→∞; as x→∞, f(x)→∞"),
    ])

def iter_level4_specs():
    return _emit("level_4", [
        ("(x-1)^4(x+2)", "as x→-∞, f(x)→-∞; as x→∞, f(x)→∞"),
        ("-(x+3)^2(x-4)^3", "as x→-∞, f(x)→∞; as x→∞, f(x)→-∞"),
    ])

def iter_level5_specs():
    return _emit("level_5", [
        ("-2(x^2+1)(x-5)^4", "as x→-∞, f(x)→-∞; as x→∞, f(x)→-∞"),
        ("(3x-1)^3(x+2)^2", "as x→-∞, f(x)→-∞; as x→∞, f(x)→∞"),
        ("-(x-1)^2(x+1)^2(x+4)", "as x→-∞, f(x)→∞; as x→∞, f(x)→-∞"),
    ])

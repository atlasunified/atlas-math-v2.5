...
MODULE_INFO = {
    "module_id": "precalculus.function_inverse_basic",
    "name": "Function Inverse Basic",
    "topic": "precalculus",
...
INSTRUCTIONS = [
    "Find the inverse function: {problem}",
    "Determine f^(-1)(x): {problem}",
    "Solve for the inverse: {problem}",
]

def _emit(level, defs):
    cap = LEVEL_SPEC_CAPS[level]
    for idx, (expr, inv) in enumerate(defs):
        if idx >= cap:
            return
        prompt = f"Find the inverse of f(x) = {expr}."
        yield _spec("inverse_basic", (expr,), prompt, inv)

def iter_level1_specs(): return _emit("level_1", [("x+3", "x-3"), ("x-5", "x+5")])
def iter_level2_specs(): return _emit("level_2", [("2x", "x/2"), ("3x-6", "(x+6)/3")])
def iter_level3_specs(): return _emit("level_3", [("x/4", "4x"), ("(x+2)/5", "5x-2")])
def iter_level4_specs(): return _emit("level_4", [("-x+7", "-x+7"), ("-2x+4", "(4-x)/2")])
def iter_level5_specs(): return _emit("level_5", [("(3x-1)/2", "(2x+1)/3"), ("(5-x)/4", "5-4x")])

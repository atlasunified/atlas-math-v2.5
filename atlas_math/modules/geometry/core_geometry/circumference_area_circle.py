...
TASK_INSTRUCTION = 'Use circle formulas with π = 3.14 unless noted'
MODULE_INFO = {'module_id': 'geometry.circumference_area_circle', 'name': 'Circumference Area Circle', 'topic': 'geometry', 'subtopic': 'core_geometry', 'difficulty_levels': ['level_1', 'level_2', 'level_3', 'level_4', 'level_5'], 'enabled': True}
LEVEL_SPEC_CAPS = {'level_1': 500, 'level_2': 700, 'level_3': 800, 'level_4': 900, 'level_5': 1000}
FAMILY_CAPS = {'level_1': {'circumference_radius': 500}, 'level_2': {'area_radius': 700}, 'level_3': {'circumference_diameter': 800}, 'level_4': {'area_diameter': 900}, 'level_5': {'compare_circle_measurements': 1000}}
CAPACITY_HINTS = {k: {'value': v, 'quality': 'capped'} for k, v in LEVEL_SPEC_CAPS.items()}
CURRICULUM = {'level_1': ['circumference from radius'], 'level_2': ['area from radius'], 'level_3': ['circumference from diameter'], 'level_4': ['area from diameter'], 'level_5': ['compare two circles numerically']}


def iter_level1_specs():
    emitted = 0; limit = FAMILY_CAPS['level_1']['circumference_radius']
    for r in range(1, 81):
        yield _spec('circumference_radius', (r,), f"A circle has radius {r}. Using π = 3.14, what is its circumference?", _num(2 * PI * r))
        emitted += 1
        if emitted >= limit:
            return
...
def iter_level2_specs():
    emitted = 0; limit = FAMILY_CAPS['level_2']['area_radius']
    for r in range(1, 61):
        yield _spec('area_radius', (r,), f"A circle has radius {r}. Using π = 3.14, what is its area?", _num(PI * r * r))
        emitted += 1
        if emitted >= limit:
            return
...
def iter_level3_specs():
    emitted = 0; limit = FAMILY_CAPS['level_3']['circumference_diameter']
    for d in range(2, 162, 2):
        yield _spec('circumference_diameter', (d,), f"A circle has diameter {d}. Using π = 3.14, what is its circumference?", _num(PI * d))
        emitted += 1
        if emitted >= limit:
            return
...
def iter_level4_specs():
    emitted = 0; limit = FAMILY_CAPS['level_4']['area_diameter']
    for d in range(2, 122, 2):
        r = d / 2
        yield _spec('area_diameter', (d,), f"A circle has diameter {d}. Using π = 3.14, what is its area?", _num(PI * r * r))
        emitted += 1
        if emitted >= limit:
            return
...
def iter_level5_specs():
    emitted = 0; limit = FAMILY_CAPS['level_5']['compare_circle_measurements']
    for r1 in range(1, 31):
        for r2 in range(r1 + 1, 41):
            diff = PI * (r2 * r2 - r1 * r1)
            yield _spec('compare_circle_measurements', (r1, r2), f"Circle A has radius {r1}. Circle B has radius {r2}. Using π = 3.14, what is the positive difference between their areas?", _num(diff))
            emitted += 1
            if emitted >= limit:
                return

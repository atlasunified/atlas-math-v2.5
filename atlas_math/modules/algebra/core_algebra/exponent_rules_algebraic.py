...
MODULE_INFO = {"module_id": "algebra.exponent_rules_algebraic", "name": "Exponent Rules Algebraic", "topic": "algebra", "subtopic": "core_algebra", "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"], "enabled": True}
LEVEL_SPEC_CAPS = {"level_1": 700, "level_2": 850, "level_3": 1000, "level_4": 1200, "level_5": 1400}
FAMILY_CAPS = {"level_1": {"product_like_bases": 700}, "level_2": {"quotient_like_bases": 850}, "level_3": {"power_of_power": 1000}, "level_4": {"negative_exponents": 1200}, "level_5": {"mixed_rules": 1400}}
...
def _sample_from_spec(spec,difficulty,instruction_idx=0):
    ...
    instruction=INSTRUCTIONS[instruction_idx % len(INSTRUCTIONS)].format(instruction="Simplify using exponent rules", problem=spec['problem'])
...
def iter_level1_specs():
    ...
    for var in ['x','y','a','b']:
        for m in range(1,13):
            for n in range(1,13):
                yield _spec('product_like_bases',(var,m,n),f"Simplify {var}^{m} · {var}^{n}.",f"{var}^{m+n}")
                emitted+=1
                if emitted>=FAMILY_CAPS['level_1']['product_like_bases']: return
...
def iter_level2_specs():
    ...
    for var in ['x','y','a','b']:
        for m in range(2,16):
            for n in range(1,m):
                yield _spec('quotient_like_bases',(var,m,n),f"Simplify {var}^{m} / {var}^{n}.",f"{var}^{m-n}")
                emitted+=1
                if emitted>=FAMILY_CAPS['level_2']['quotient_like_bases']: return
...
def iter_level3_specs():
    ...
    for var in ['x','y','a','b']:
        for m in range(2,10):
            for n in range(2,10):
                yield _spec('power_of_power',(var,m,n),f"Simplify ({var}^{m})^{n}.",f"{var}^{m*n}")
                emitted+=1
                if emitted>=FAMILY_CAPS['level_3']['power_of_power']: return

def iter_level4_specs():
    emitted=0
    for var in ['x','y','a','b']:
        for n in range(1,13):
            yield _spec('negative_exponents',(var,n),f"Rewrite {var}^-{n} using only positive exponents.",f"1/{var}^{n}")
            emitted+=1
            if emitted>=FAMILY_CAPS['level_4']['negative_exponents']: return
            ...
            for q in range(1,8):
                for r in range(1,8):
                    ...
                    if ans_exp == 0:
                        ...
                        ans = f"1/{var}^{-ans_exp}"
                    yield _spec('mixed_rules',(var,p,q,r),f"Simplify ({var}^{p} · {var}^{q}) / {var}^{r}.",ans)
                    emitted+=1
                    if emitted>=FAMILY_CAPS['level_5']['mixed_rules']: return
...
def curriculum(): return {'level_1':['apply product rules'],'level_2':['apply quotient rules'],'level_3':['apply power-of-a-power rules'],'level_4':['rewrite negative exponents'],'level_5':['combine exponent rules in one expression']}

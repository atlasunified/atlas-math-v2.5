...
MODULE_INFO = {"module_id": "algebra.function_notation_basic", "name": "Function Notation Basic", "topic": "algebra", "subtopic": "core_algebra", "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"], "enabled": True}
LEVEL_SPEC_CAPS = {"level_1": 700, "level_2": 900, "level_3": 1100, "level_4": 1300, "level_5": 1500}
FAMILY_CAPS = {"level_1": {"evaluate_linear": 700}, "level_2": {"evaluate_polynomial": 900}, "level_3": {"solve_for_input": 1100}, "level_4": {"difference_quotient_basic": 1300}, "level_5": {"compose_basic": 1500}}
...
def _sample_from_spec(spec,difficulty,instruction_idx=0):
    ...
    instruction=INSTRUCTIONS[instruction_idx % len(INSTRUCTIONS)].format(instruction="Use function notation correctly", problem=spec['problem'])
...
def iter_level1_specs():
    emitted=0; limit=FAMILY_CAPS['level_1']['evaluate_linear']
    for a in range(-6,7):
        if a==0: continue
        for b in range(-10,11):
            for x in range(-8,9):
                yield _spec('evaluate_linear',(a,b,x),f"If f(x) = {a}x + {b}, find f({x}).",str(a*x+b))
                emitted+=1
                if emitted>=limit:return

def iter_level2_specs():
    emitted=0; limit=FAMILY_CAPS['level_2']['evaluate_polynomial']
    for a in range(1,5):
        for b in range(-5,6):
            for c in range(-5,6):
                for x in range(-4,5):
                    yield _spec('evaluate_polynomial',(a,b,c,x),f"If g(x) = {a}x^2 + {b}x + {c}, find g({x}).",str(a*x*x+b*x+c))
                    emitted+=1
                    if emitted>=limit:return

def iter_level3_specs():
    emitted=0; limit=FAMILY_CAPS['level_3']['solve_for_input']
    for a in [-5,-4,-3,-2,-1,1,2,3,4,5]:
        for b in range(-10,11):
            for x in range(-8,9):
                y=a*x+b
                yield _spec('solve_for_input',(a,b,y),f"If h(x) = {a}x + {b}, find x when h(x) = {y}.",str(x))
                emitted+=1
                if emitted>=limit:return

def iter_level4_specs():
    emitted=0; limit=FAMILY_CAPS['level_4']['difference_quotient_basic']
    for a in [-4,-3,-2,-1,1,2,3,4]:
        for b in range(-8,9):
            for x in range(-5,6):
                yield _spec('difference_quotient_basic',(a,b,x),f"If p(x) = {a}x + {b}, compute [p({x}+1) - p({x})].",str(a))
                emitted+=1
                if emitted>=limit:return

def iter_level5_specs():
    emitted=0; limit=FAMILY_CAPS['level_5']['compose_basic']
    for a1 in [-3,-2,-1,1,2,3]:
        for b1 in range(-4,5):
            for a2 in [-3,-2,-1,1,2,3]:
                for b2 in range(-4,5):
                    for x in range(-3,4):
                        val=a1*(a2*x+b2)+b1
                        yield _spec('compose_basic',(a1,b1,a2,b2,x),f"If f(x) = {a1}x + {b1} and g(x) = {a2}x + {b2}, find f(g({x})).",str(val))
                        emitted+=1
                        if emitted>=limit:return
...
def curriculum(): return {'level_1':['evaluate linear functions'],'level_2':['evaluate quadratic-style expressions in function notation'],'level_3':['solve for an input from a function value'],'level_4':['work simple difference quotients'],'level_5':['compose basic linear functions']}

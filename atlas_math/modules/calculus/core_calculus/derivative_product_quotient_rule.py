...
MODULE_INFO = {
    "module_id": "calculus.derivative_product_quotient_rule",
    "name": "Derivative Product Quotient Rule",
    "topic": "calculus",
...
INSTRUCTIONS = [
    "Find the derivative using the product or quotient rule: {problem}",
    "Differentiate carefully: {problem}",
    "Compute the derivative and simplify lightly: {problem}",
]
...
def _emit(level: str, items):
    ...
    for idx, item in enumerate(items):
        ...
        yield _spec(family, values, f"Find d/dx of {expr}.", ans, {"expression": expr})
...
def iter_level1_specs():
    ...
    for a in [1,2,3,4]:
        for b in [1,2,3,5]:
            expr=f"({a}x)({b}x^2)"
            ans=f"{3*a*b}x^2"
            items.append((expr,ans,'product_monomials',(a,b)))
...
def iter_level2_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,4]:
            for c in [1,3,5]:
                expr=f"({a}x+{b})(x^{2}+{c})"
                ans=f"{a}(x^2+{c}) + ({a}x+{b})(2x)"
                items.append((expr,ans,'product_linear_quadratic',(a,b,c)))
...
def iter_level3_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,4]:
            for c in [1,3,5]:
                expr=f"({a}x+{b})/(x+{c})"
                ans=f"({a}(x+{c}) - ({a}x+{b}))/(x+{c})^2"
                items.append((expr,ans,'quotient_linear_linear',(a,b,c)))
...
def iter_level4_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,3]:
            for c in [1,2,4]:
                expr=f"(x^{2}+{a})({b}x^{3}-{c})"
                ans=f"(2x)({b}x^3-{c}) + (x^2+{a})({3*b}x^2)"
                items.append((expr,ans,'product_poly_poly',(a,b,c)))
...
def iter_level5_specs():
    ...
    for a in [1,2]:
        for b in [1,3,5]:
            for c in [1,2,4]:
                expr=f"(x^{2}+{a})/(x^{2}+{b}x+{c})"
                ans=f"(2x)(x^2+{b}x+{c}) - (x^2+{a})(2x+{b}) over (x^2+{b}x+{c})^2"
                items.append((expr,ans,'quotient_quadratic_quadratic',(a,b,c)))

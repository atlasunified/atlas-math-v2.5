...
MODULE_INFO = {
    "module_id": "calculus.differential_equations_separable_basic",
    "name": "Differential Equations Separable Basic",
    "topic": "calculus",
    "subtopic": "core_calculus",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}
INSTRUCTIONS = ['Solve the differential equation: {problem}', 'Find the general solution: {problem}', 'Separate variables and solve: {problem}']
...
def iter_level_1_specs():
    ...
    for k in [1,2,3,4,5]:
        prompt=f"dy/dx={k}"
        items.append(("constant_rate", (k,), prompt, f"y={k}x+C", {"ode_type":"constant"}))
...
def iter_level_2_specs():
    ...
    for k in [1,2,3,4,5]:
        prompt=f"dy/dx={k}x"
        items.append(("linear_x", (k,), prompt, f"y={k/2:g}x^2+C", {"ode_type":"x_only"}))
...
def iter_level_3_specs():
    ...
    for k in [1,2,3,4]:
        prompt=f"dy/dx={k}y"
        items.append(("exp_growth", (k,), prompt, f"y=Ce^({k}x)", {"ode_type":"y_only"}))
...
def iter_level_4_specs():
    ...
    for k in [1,2,3,4]:
        prompt=f"dy/dx={k}xy"
        items.append(("xy_separable", (k,), prompt, f"y=Ce^({k/2:g}x^2)", {"ode_type":"xy"}))
...
def iter_level_5_specs():
    ...
    for k in [1,2,3]:
        for n in [1,2,3,4]:
            prompt=f"dy/dx={k}x^{n}y"
            coeff=k/(n+1)
            items.append(("xny_separable", (k,n), prompt, f"y=Ce^({coeff:g}x^{n+1})", {"ode_type":"x_power_times_y"}))

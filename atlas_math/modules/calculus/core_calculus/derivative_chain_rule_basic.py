...
MODULE_INFO = {
    "module_id": "calculus.derivative_chain_rule_basic",
    "name": "Derivative Chain Rule Basic",
    "topic": "calculus",
...
INSTRUCTIONS = [
    "Differentiate using the chain rule: {problem}",
    "Find the derivative: {problem}",
    "Apply the chain rule carefully: {problem}",
]
...
def iter_level1_specs():
    ...
    for a in [1,2,3,4]:
        for b in [0,1,2,5]:
            for n in [2,3,4,5]:
                inner=f"{a}x{('+'+str(b)) if b else ''}"
                ans=f"{n*a}({inner})^{n-1}"
                items.append((f"({inner})^{n}",ans,'power_of_linear',(a,b,n)))
...
def iter_level2_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,4]:
            for n in [2,3,4]:
                inner=f"x^{2}+{a}x+{b}"
                ans=f"{n}({inner})^{n-1}(2x+{a})"
                items.append((f"({inner})^{n}",ans,'power_of_quadratic',(a,b,n)))
...
def iter_level3_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,5]:
            inner=f"{a}x-{b}"
            ans=f"{a}/(2sqrt({inner}))"
            items.append((f"sqrt({inner})",ans,'sqrt_linear',(a,b)))
...
def iter_level4_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,4]:
            inner=f"{a}x+{b}"
            ans=f"{a}e^({inner})"
            items.append((f"e^({inner})",ans,'exp_linear',(a,b)))
            ans2=f"{a}/({inner})"
            items.append((f"ln({inner})",ans2,'log_linear',(a,b)))
...
def iter_level5_specs():
    ...
    for a in [1,2]:
        for b in [1,3,5]:
            for n in [2,3,4]:
                inner=f"x^{2}+{a}x+{b}"
                ans=f"{n}({inner})^{n-1}(2x+{a})/(1+({inner})^{n})"
                items.append((f"ln(1+({inner})^{n})",ans,'log_of_power',(a,b,n)))

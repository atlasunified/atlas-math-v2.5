...
MODULE_INFO = {
    "module_id": "calculus.area_between_curves_basic",
    "name": "Area Between Curves Basic",
    "topic": "calculus",
    "subtopic": "core_calculus",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}
INSTRUCTIONS = ['Find the area between the curves: {problem}', 'Compute the enclosed area: {problem}', 'Evaluate the area between these functions: {problem}']
...
def iter_level_1_specs():
    ...
    for a in [1,2,3,4,5]:
        for b in [0,1,2,3]:
            ...
            upper=f"{a}x+{b}"
            lower=f"{b}"
            prompt=f"y={upper} and y={lower} on [0,2]"
            area=2*a
            items.append(("line_constant", (a,b), prompt, f"{area}", {"form":"line_minus_constant"}))
...
def iter_level_2_specs():
    ...
    for a in [1,2,3,4]:
        for b in [1,2,3]:
            prompt=f"y={a}x+{b} and y={b} on [0,3]"
            area=f"{9*a/2:g}"
            items.append(("line_constant_wide", (a,b), prompt, area, {"interval":[0,3]}))
...
def iter_level_3_specs():
    ...
    for a in [1,2,3,4]:
        for c in [1,2,3]:
            prompt=f"y=x^2+{c} and y={c} on [0,{a}]"
            area=f"{a**3/3:g}"
            items.append(("parabola_constant", (a,c), prompt, area, {"upper":"x^2+c","lower":"c"}))
...
def iter_level_4_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,3,4]:
            prompt=f"y={a}x^2+{b} and y={b} on [0,2]"
            area=f"{8*a/3:g}"
            items.append(("scaled_parabola_constant", (a,b), prompt, area, {"scale":a}))
...
def iter_level_5_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,3]:
            prompt=f"y={a}x^2+{b}x and y={b}x on [0,2]"
            area=f"{8*a/3:g}"
            items.append(("quadratic_linear", (a,b), prompt, area, {"difference":"ax^2"}))

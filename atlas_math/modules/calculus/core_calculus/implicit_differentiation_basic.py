...
MODULE_INFO = {
    "module_id": "calculus.implicit_differentiation_basic",
    "name": "Implicit Differentiation Basic",
    "topic": "calculus",
...
INSTRUCTIONS = [
    "Use implicit differentiation to find dy/dx: {problem}",
    "Differentiate implicitly: {problem}",
    "Find dy/dx from the relation: {problem}",
]
...
def _emit(level: str, items):
    ...
    for idx, item in enumerate(items):
        if idx >= cap:
            return
        relation, ans, family, values = item
        yield _spec(family, values, f"Given {relation}, find dy/dx.", ans, {"relation": relation})

def iter_level1_specs():
    items=[]
    for a in [1,2,3,4]:
        for b in [1,2,5]:
            relation=f"{a}x + {b}y = 7"
            ans=f"-{a}/{b}"
            items.append((relation,ans,'linear_relation',(a,b)))
...
def iter_level2_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,4]:
            relation=f"x^2 + {a}y^2 = {b}"
            ans=f"-x/({a}y)"
            items.append((relation,ans,'ellipse_form',(a,b)))
...
def iter_level3_specs():
    ...
    for a in [1,2,3]:
        for b in [1,3,5]:
            relation=f"x^2 + {a}xy + y^2 = {b}"
            ans=f"-(2x + {a}y)/({a}x + 2y)"
            items.append((relation,ans,'xy_mixed',(a,b)))
...
def iter_level4_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,4]:
            relation=f"x^3 + y^3 = {a}xy + {b}"
            ans=f"({a}y - 3x^2)/(3y^2 - {a}x)"
            items.append((relation,ans,'cubic_with_xy',(a,b)))
...
def iter_level5_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,5]:
            relation=f"e^y + x^2y = {a}x + {b}"
            ans=f"({a} - 2xy)/(e^y + x^2)"
            items.append((relation,ans,'exp_and_product',(a,b)))

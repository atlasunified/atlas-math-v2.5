...
MODULE_INFO = {
    "module_id": "calculus.first_derivative_sign_analysis",
    "name": "First Derivative Sign Analysis",
    "topic": "calculus",
...
INSTRUCTIONS = [
    "Use the first derivative to analyze monotonicity: {problem}",
    "Determine increasing/decreasing intervals: {problem}",
    "Identify critical points and the sign pattern: {problem}",
]
...
def iter_level1_specs():
    ...
    for c in [-5,-3,-2,-1,1,2,3,5]:
        prompt=f"For f'(x)=x-{c}, determine where f is increasing and decreasing."
        ans=f"decreasing on (-∞, {c}); increasing on ({c}, ∞)"
        items.append(("linear_derivative", (c,), prompt, ans, {"critical_points":[c]}))
    return _emit('level_1', items)

def iter_level2_specs():
    items=[]
    for c in [-4,-2,0,2,4]:
        prompt=f"For f'(x)=2(x-{c}), determine where f is increasing and decreasing."
        ans=f"decreasing on (-∞, {c}); increasing on ({c}, ∞)"
        items.append(("scaled_linear_derivative", (c,), prompt, ans, {"critical_points":[c]}))
...
def iter_level3_specs():
    ...
    for a in [1,2,3]:
        for b in [3,4,5,6]:
            prompt=f"For f'(x)=(x-{a})(x-{b}), determine where f is increasing and decreasing."
            ans=f"increasing on (-∞, {a}) and ({b}, ∞); decreasing on ({a}, {b})"
            items.append(("two_roots_positive_lead", (a,b), prompt, ans, {"critical_points":[a,b]}))
...
def iter_level4_specs():
    ...
    for a in [1,2,3]:
        for b in [4,5,6,7]:
            prompt=f"For f'(x)=-(x-{a})(x-{b}), determine where f is increasing and decreasing."
            ans=f"decreasing on (-∞, {a}) and ({b}, ∞); increasing on ({a}, {b})"
            items.append(("two_roots_negative_lead", (a,b), prompt, ans, {"critical_points":[a,b]}))
...
def iter_level5_specs():
    ...
    for a in [1,2,3]:
        for b in [4,5,6]:
            for c in [7,8,9]:
                prompt=f"For f'(x)=(x-{a})(x-{b})(x-{c}), determine where f is increasing and decreasing."
                ans=f"decreasing on (-∞, {a}) and ({b}, {c}); increasing on ({a}, {b}) and ({c}, ∞)"
                items.append(("three_roots_positive_lead", (a,b,c), prompt, ans, {"critical_points":[a,b,c]}))

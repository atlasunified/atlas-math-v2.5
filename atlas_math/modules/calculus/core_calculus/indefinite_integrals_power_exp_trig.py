...
MODULE_INFO = {
    "module_id": "calculus.indefinite_integrals_power_exp_trig",
    "name": "Indefinite Integrals Power Exp Trig",
    "topic": "calculus",
...
INSTRUCTIONS = [
    "Find the indefinite integral: {problem}",
    "Evaluate the antiderivative: {problem}",
    "Integrate and include the constant of integration: {problem}",
]
...
def iter_level1_specs():
    ...
    for a in [1,2,3,4,5,6]:
        for n in [0,1,2,3,4,5]:
            prompt=f"∫ {a}x^{n} dx"
            ans=f"{a/(n+1):g}x^{n+1}+C" if a % (n+1) else f"{a//(n+1)}x^{n+1}+C"
            items.append(("power_basic", (a,n), prompt, ans, {"integrand_type":"power"}))
...
def iter_level2_specs():
    ...
    for a in [1,2,3,4,5]:
        for n in [1,2,3,4,5]:
            prompt=f"∫ {a}x^{n} dx"
            coeff = a/(n+1)
            ans=f"{coeff:g}x^{n+1}+C"
            items.append(("power_fraction_coeff", (a,n), prompt, ans, {"integrand_type":"power"}))
        prompt=f"∫ {a}e^x dx"
        items.append(("exp_basic", (a,), prompt, f"{a}e^x+C", {"integrand_type":"exponential"}))
...
def iter_level3_specs():
    ...
    for a in [1,2,3,4,5]:
        for b in [1,2,3,4]:
            prompt=f"∫ {a}e^({b}x) dx"
            ans=f"{a/b:g}e^({b}x)+C"
            items.append(("exp_linear", (a,b), prompt, ans, {"integrand_type":"exponential"}))
    for a in [1,2,3,4,5]:
        prompt=f"∫ {a}sin(x) dx"
        items.append(("sin_basic", (a,), prompt, f"-{a}cos(x)+C", {"integrand_type":"trig"}))
        prompt=f"∫ {a}cos(x) dx"
        items.append(("cos_basic", (a,), prompt, f"{a}sin(x)+C", {"integrand_type":"trig"}))
...
def iter_level4_specs():
    ...
    for a in [1,2,3,4]:
        for b in [2,3,4,5]:
            prompt=f"∫ {a}sin({b}x) dx"
            ans=f"-{a/b:g}cos({b}x)+C"
            items.append(("sin_linear", (a,b), prompt, ans, {"integrand_type":"trig"}))
            prompt=f"∫ {a}cos({b}x) dx"
            ans=f"{a/b:g}sin({b}x)+C"
            items.append(("cos_linear", (a,b), prompt, ans, {"integrand_type":"trig"}))
...
def iter_level5_specs():
    ...
    for a in [1,2,3]:
        for b in [1,2,3,4]:
            for n in [1,2,3,4]:
                prompt=f"∫ ({a}x^{n} + {b}e^x) dx"
                ans=f"{a/(n+1):g}x^{n+1}+{b}e^x+C"
                items.append(("sum_power_exp", (a,n,b), prompt, ans, {"integrand_type":"sum"}))

...
MODULE_INFO = {
    "module_id": "calculus.sequences_series_intro",
    "name": "Sequences Series Intro",
    "topic": "calculus",
    "subtopic": "core_calculus",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}
INSTRUCTIONS = ['Find the requested value: {problem}', 'Evaluate the sequence or series expression: {problem}', 'Compute the result: {problem}']
...
def iter_level_1_specs():
    ...
    for a1 in [1,2,3,4,5]:
        for d in [1,2,3,4]:
            for n in [2,3,4,5,6]:
                prompt=f"For the arithmetic sequence with a_1={a1} and d={d}, find a_{n}."
                items.append(("arith_term", (a1,d,n), prompt, f"{a1+(n-1)*d}", {"sequence_type":"arithmetic"}))
...
def iter_level_2_specs():
    ...
    for a1 in [1,2,3,4]:
        for r in [2,3,4]:
            for n in [2,3,4,5]:
                prompt=f"For the geometric sequence with a_1={a1} and r={r}, find a_{n}."
                items.append(("geom_term", (a1,r,n), prompt, f"{a1*(r**(n-1))}", {"sequence_type":"geometric"}))
...
def iter_level_3_specs():
    ...
    for a1 in [1,2,3,4]:
        for d in [1,2,3,4]:
            for n in [3,4,5,6]:
                prompt=f"For the arithmetic sequence with a_1={a1} and d={d}, find S_{n}."
                s=n*(2*a1+(n-1)*d)/2
                items.append(("arith_sum", (a1,d,n), prompt, f"{s:g}", {"sequence_type":"arithmetic_sum"}))
...
def iter_level_4_specs():
    ...
    for a1 in [1,2,3]:
        for r in [2,3,4]:
            for n in [3,4,5]:
                prompt=f"For the geometric sequence with a_1={a1} and r={r}, find S_{n}."
                s=a1*(r**n-1)/(r-1)
                items.append(("geom_sum", (a1,r,n), prompt, f"{s:g}", {"sequence_type":"geometric_sum"}))
...
def iter_level_5_specs():
    ...
    for a in [1,2,3,4]:
        for b in [0,1,2,3]:
            for n in [4,5,6,7]:
                prompt=f"Given a_n={a}n+{b}, find a_{n} and S_{n}."
                term=a*n+b
                s=sum(a*k+b for k in range(1,n+1))
                items.append(("explicit_linear_sequence", (a,b,n), prompt, f"a_{n}={term}, S_{n}={s}", {"sequence_type":"explicit_linear"}))

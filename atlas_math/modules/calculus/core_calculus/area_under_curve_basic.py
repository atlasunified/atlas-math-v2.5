...
MODULE_INFO = {
    "module_id": "calculus.area_under_curve_basic",
    "name": "Area Under Curve Basic",
    "topic": "calculus",
...
INSTRUCTIONS = [
    "Find the area under the curve: {problem}",
    "Compute the indicated area: {problem}",
    "Evaluate the geometric/calculus area: {problem}",
]
...
def iter_level1_specs():
    ...
    for b in [1,2,3,4,5,6]:
        prompt=f"Find the area under y=x from x=0 to x={b}."
        ans=f"{b*b/2:g}"
        items.append(("line_origin", (b,), prompt, ans, {"curve":"y=x"}))
...
def iter_level2_specs():
    ...
    for a in [1,2,3,4,5]:
        for b in [1,2,3,4]:
            prompt=f"Find the area under y={a} from x=0 to x={b}."
            ans=str(a*b)
            items.append(("constant_area", (a,b), prompt, ans, {"curve":"constant"}))
...
def iter_level3_specs():
    ...
    for a in [1,2,3,4]:
        for b in [1,2,3,4]:
            prompt=f"Find the area under y={a}x^2 from x=0 to x={b}."
            ans=f"{a*b**3/3:g}"
            items.append(("quadratic_origin", (a,b), prompt, ans, {"curve":"quadratic"}))
...
def iter_level4_specs():
    ...
    for m in [1,2,3,4]:
        for c in [0,1,2,3]:
            for b in [1,2,3]:
                prompt=f"Find the area under y={m}x+{c} from x=0 to x={b}."
                ans=f"{(m*b*b/2)+(c*b):g}"
                items.append(("linear_area", (m,c,b), prompt, ans, {"curve":"linear"}))
    return _emit("level_4", items)

def iter_level5_specs():
    items=[]
    for b in [1,2,3,4]:
        prompt=f"Find the area under y=e^x from x=0 to x={b}."
        ans={1:"e-1",2:"e^2-1",3:"e^3-1",4:"e^4-1"}[b]
        items.append(("exp_area", (b,), prompt, ans, {"curve":"exponential"}))
    for b in ["π/2","π"]:
        prompt=f"Find the area under y=sin(x) from x=0 to x={b}."
        ans={"π/2":"1","π":"2"}[b]
        items.append(("sin_area", (b,), prompt, ans, {"curve":"trig"}))
